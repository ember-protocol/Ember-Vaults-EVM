...
/**
 * Sets fee exemption list status for one user on one vault.
 *
 ...
 */
async function main() {
  console.log("\n📝 Setting Vault Fee Exemption List Status...\n");
  ...
  const currentStatus = await validator.feeExemptAccounts(vaultInfo.proxyAddress, userAddress);
  ...
  if (currentStatus === status) {
    console.log("✅ Fee exemption list status already set. No action needed.");
    process.exit(0);
  }

  const protocolConfig = await ethers.getContractAt("EmberProtocolConfig", protocolConfigAddress);
  const tx = await protocolConfig.setVaultFeeExemptionList(
    vaultInfo.proxyAddress,
    userAddress,
    status
  );
  ...
  const updatedStatus = await validator.feeExemptAccounts(vaultInfo.proxyAddress, userAddress);
  if (updatedStatus !== status) {
    console.error("❌ Verification failed for fee exemption list update");
    process.exit(1);
  }

  console.log("✅ Fee exemption list status updated successfully:", updatedStatus);
}
